...
func (g *Graph) ShortestAll(src, dest string) (BestPaths, error) {
	return g.evaluateAll(src, dest, true)
}

//LongestAll calculates all the longest paths from src to dest
func (g *Graph) LongestAll(src, dest string) (BestPaths, error) {
	return g.evaluateAll(src, dest, false)
}

func (g *Graph) evaluateAll(src, dest string, shortest bool) (BestPaths, error) {
	//Setup graph
	g.setup(shortest, src, -1)
	return g.postSetupEvaluateAll(src, dest, shortest)
}

func (g *Graph) postSetupEvaluateAll(src, dest string, shortest bool) (BestPaths, error) {
	var (
		current    *Vertex
		oldCurrent string
	)
		...
		for v, dist := range current.arcs {
			...
			if (shortest && current.distance+dist < g.Verticies[v].distance) ||
				...
				if current.distance+dist == g.Verticies[v].distance {
					...
					g.Verticies[v].bestVerticies = []string{current.ID}
				}
				if v == dest {
					g.visitedDest = true
...
func (g *Graph) bestPaths(src, dest string) BestPaths {
	var (
		paths = g.visitPath(src, dest, dest, [][]string{})
		best  BestPaths
	)
	...
	for indexPaths := range paths {
		...
		best = append(best, BestPath{
			Distance: g.Verticies[dest].distance,
			Path:     paths[indexPaths],
		})
	}
...
func (g *Graph) visitPath(src, dest, currentNode string, paths [][]string) [][]string {
	if currentNode == src {
